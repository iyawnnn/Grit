<div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 relative" 
     x-data="{ showUpload: false, showDeleteModal: false, resumeToDelete: null }" 
                        ...
                        <div wire:key="resume-card-{{ $resume->id }}"
                            ...
                            <div class="pt-5 border-t border-gray-100 flex items-center justify-between mt-auto shrink-0">
                                <div class="flex gap-4">
                                    <a href="{{ $resume->file_url }}" target="_blank" class="text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors flex items-center gap-1.5">
                                        <x-heroicon-o-arrow-top-right-on-square class="w-4 h-4" /> PDF
                                    </a>
                                    <a href="{{ route('resumes.show', $resume) }}" class="text-sm font-bold text-gray-500 hover:text-[#e26a35] transition-colors flex items-center gap-1.5">
                                        <x-heroicon-o-code-bracket class="w-4 h-4" /> Data
                                    </a>
                                </div>

                                <button type="button" @click="resumeToDelete = {{ $resume->id }}; showDeleteModal = true"
        ...
        <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div x-show="showDeleteModal"
                     ...
                     class="relative transform overflow-hidden rounded-2xl bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6 border border-gray-100">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                            <h3 class="text-base font-extrabold leading-6 text-gray-900" id="modal-title">Delete Resume</h3>
                            <div class="mt-2">
                                <p class="text-sm font-medium text-gray-500">Are you sure you want to remove this resume? It will be permanently deleted from our servers and Cloudinary. This action cannot be undone.</p>
                            </div>
                        </div>
                    </div>
                    <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse gap-3">
                        <button type="button" @click="$wire.executeDelete(resumeToDelete); showDeleteModal = false" wire:loading.attr="disabled" class="inline-flex w-full justify-center rounded-xl bg-red-600 px-3 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-red-500 sm:w-auto transition-colors disabled:opacity-70">
                            Delete
                        </button>
                        <button type="button" @click="showDeleteModal = false" class="mt-3 inline-flex w-full justify-center rounded-xl bg-white px-3 py-2.5 text-sm font-bold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto transition-colors">
                            Cancel
                        </button>
                    </div>
                </div>
